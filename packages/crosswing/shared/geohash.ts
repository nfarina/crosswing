// Copied from https://github.com/firebase/geofire-js/blob/master/packages/geofire-common/src/index.ts
// Trimmed down to only the parts we need.
 ...
 */
function metersToLongitudeDegrees(distance: number, latitude: number): number {
  const radians = deg2rad(latitude);
 ...
 */
function boundingBoxBits(coordinate: Geopoint, size: number): number {
 ...
 */
function geohashQuery(geohash: Geohash, bits: number): GeohashRange {
