...
export class AsyncPool {
  ...
  // Track number of calls for debugging.
  private nextNum: number = 1;

  // Timestamp of last X calls to fn(), where X === rate.
  ...
  // Any error encountered while executing tasks.
  private error: Error | null = null;

  // Next caller of start() that's waiting.
  private addPromise: PromiseControl | null = null;

  // If someone called finish() while we still had tasks active, this is
  // who we tell when we're completely finished.
  private finishPromise: PromiseControl | null = null;

  private started: number = Date.now();

  // Set to true when we are completely done.
